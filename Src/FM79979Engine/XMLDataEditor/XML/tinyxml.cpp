...
bool TiXmlDocument::LoadFile( const WCHAR* _filename, TiXmlEncoding encoding )
{
	...
	WideCharToMultiByte( CP_ACP, 0, value.c_str (), -1, l_strFileName, 256, NULL, NULL );//q for return p for convert
...
bool TiXmlDocument::LoadFile( FILE* file, TiXmlEncoding encoding )
{
	...
	// both the two-WCHARacter sequence #xD #xA and any #xD that is not followed by #xA to
	// a single #xA WCHARacter.
	// </quote>
	//
	...
	*/
	...
	while( *p ) {
		assert( p < (buf+length) );
		if ( *p == 0xa ) {
			// Newline WCHARacter. No special rules for this. Append all the WCHARacters
			...
			data += (WCHAR)0xa;						// a proper newline
			...
			if ( *(p+1) == 0xa ) {
				...
				// it was followed by something else...that is presumably WCHARacters again.
				++p;
				lastPos = p;
				assert( p <= (buf+length) );
			}
	...
	// Handle any left over WCHARacters.
	if ( p-lastPos ) {
...
bool TiXmlDocument::SaveFile( const WCHAR* filename ) const
{
	// The old c stuff lives on...
		char	l_strFileName[MAX_PATH];
	WideCharToMultiByte( CP_ACP, 0, filename, -1, l_strFileName, 256, NULL, NULL );//q for return p for convert
	FILE* fp = TiXmlFOpen( l_strFileName, "w" );
	if ( fp )
	{
