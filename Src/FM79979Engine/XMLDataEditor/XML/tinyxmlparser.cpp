	...
	while ( p < now )
	{
		...
		switch (*pU) {
			...
			default:
				if ( encoding == TIXML_ENCODING_UTF8 )
				{
					// Eat the 1 to 4 byte utf8 WCHARacter.
					int step = (int)*(WCHAR*)p;
					if ( step == 0 )
...
/*static*/ bool TiXmlBase::StreamTo( std::istream * in, int WCHARacter, TIXML_STRING * tag )
{
	//assert( WCHARacter > 0 && WCHARacter < 128 );	// else it won't work in utf-8
	while ( in->good() )
	{
		int c = in->peek();
		if ( c == WCHARacter )
		...
		for ( ;; )
		{
			...
			for( ;; )
			{
				...
				*tag += (WCHAR) c;
				...
				{
					size_t len = tag->size();
					const WCHAR* start = tag->c_str() + len - 9;
					if ( wcscmp( start, "<![CDATA[" ) == 0 ) {
						assert( !closingTag );
						break;
					}
				}

				if ( !firstWCHARFound && c != '<' && !IsWhiteSpace( c ) )
				{
					firstWCHARFound = true;
					if ( c == '/' )
						closingTag = true;
				}
			}
