...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    l_BS->GlVersion = (GLuint)(major * 100 + minor * 10);
#if defined(GL_CONTEXT_PROFILE_MASK)
    if (l_BS->GlVersion >= 320)
        glGetIntegerv(GL_CONTEXT_PROFILE_MASK, &l_BS->GlProfileMask);
    l_BS->GlProfileIsCompat = (l_BS->GlProfileMask & GL_CONTEXT_COMPATIBILITY_PROFILE_BIT) != 0;
#endif
    ...
    if (strncmp(gl_version_str, "OpenGL ES 3", 11) == 0)
        l_BS->GlProfileIsES3 = true;
#endif

    l_BS->UseBufferSubData = false;
    /*
    ...
    */
    ...
    IM_ASSERT((int)strlen(glsl_version) + 2 < IM_ARRAYSIZE(l_BS->GlslVersionString));
    strcpy(l_BS->GlslVersionString, glsl_version);
    strcat(l_BS->GlslVersionString, "\n");
    ...
    l_BS->HasPolygonMode = (!l_BS->GlProfileIsES2 && !l_BS->GlProfileIsES3);
#endif
    l_BS->HasClipOrigin = (l_BS->GlVersion >= 450);
