...
/*A Encode member function*/
BOOL oggHelper::Encode(char* file_in, char* file_out)
{
	VorbisComment ivc;
	EncodeSetting es;
	return encode_x(file_in, file_out, es, ivc, NULL);
}

/*A Encode member function*/
BOOL oggHelper::Encode(char* file_in, char* file_out, EncodeSetting es)
{
	VorbisComment ivc;
	return encode_x(file_in, file_out, es, ivc, NULL);
}
...
/*A Encode member function*/
BOOL oggHelper::Encode(char* file_in, char* file_out, EncodeSetting es, VorbisComment ivc)
{
	return encode_x(file_in, file_out, es, ivc, NULL);
}

/*A Encode member function*/
BOOL oggHelper::Encode(char* file_in, char* file_out, EncodeSetting es, VorbisComment ivc, WNDPROC callbackproc)
{
	return encode_x(file_in, file_out, es, ivc, callbackproc);
}

/*A Encode member function*/ //The asynchronous version
void* oggHelper::Encode(char* file_in, char* file_out, EncodeSetting es, VorbisComment ivc, WNDPROC callbackproc, BOOL async = FALSE)
{
	if(async)
	{
		for(int i = 0; i < MAX_THREAD_COUNT; i++)//Trying to find a free thread to use
		{
			if((WaitForSingleObject(hThread[i], 0) == WAIT_FAILED && GetLastError() == ERROR_INVALID_HANDLE)/*Not used before*/ || WaitForSingleObject(hThread[i], 0) == WAIT_OBJECT_0/*Finished and ready for use again*/)
			{
				...
				hSParam[i] = new OGG_StaticParam_t;
				OGG_StaticParam_t* holder = hSParam[i];
				holder->callback_proc = callbackproc;
				holder->oggObj = this;
				holder->ogg = file_out;
				holder->pcm = file_in;
				holder->settings = es;
				holder->comments = ivc;
				return hThread[i] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)encode_s, (void*)holder, NULL, NULL);
			}
		}
	}
	else
	{
		return (void*)encode_x(file_in, file_out, es, ivc, callbackproc);
	}
	return (void*)-3;//Thread as reached maximum allowed, wait for one to finish and try again
}

//The static member which we can use for the thread
int oggHelper::encode_s(void* param)
{
	OGG_StaticParam_t* sp = (OGG_StaticParam_t*)param;
	char* file_in = sp->pcm;
	char* file_out = sp->ogg;
	EncodeSetting settings = sp->settings;
	VorbisComment comments = sp->comments;
	WNDPROC callback_proc = sp->callback_proc;
	oggHelper* oh = (oggHelper*)sp->oggObj;
    return oh->encode_x(file_in, file_out, settings, comments, callback_proc);
}

/*The actual Encode function*/
BOOL oggHelper::encode_x(char* file_in, char* file_out, EncodeSetting es, VorbisComment ivc, WNDPROC callbackproc)
{
	...
	if(!wfh->IsWAVE(f_in, wfh))
	{
		...
		if(callbackproc != NULL)
		{
			callbackproc((HWND)GetModuleHandle(NULL), OH_ERROR, -2, NULL);
		}
		goto done;
	}
	...
	case VBR:
		{
			...
			result = vorbis_encode_init_vbr(&vi, es.channel, wfh->Fmt_SamplingRate, es.vbr_quality);
		}
		break;
	case ABR:
		{
			if(es.max_abr_br != -1)
			{
				result = (vorbis_encode_setup_managed(&vi , es.channel, wfh->Fmt_SamplingRate, es.min_abr_br, es.abr_br, es.max_abr_br) ||
					vorbis_encode_ctl(&vi, OV_ECTL_RATEMANAGE2_SET, NULL) || vorbis_encode_setup_init(&vi));
			}
			else
			{
				result = vorbis_encode_init(&vi, es.channel, wfh->Fmt_SamplingRate, -1, es.abr_br, -1);
			}
		}
		break;
	case CBR:
		result = vorbis_encode_init(&vi, es.channel, wfh->Fmt_SamplingRate, es.cbr_br, es.cbr_br, es.cbr_br);
		break;
	default:
		//Error
		{
	...
	ogg_stream_init(&os, rand());
	 ...
	 while(true)
	 {
		 ...
		 else
		 {
			 ...
			 if(wfh->Fmt_Channels == 2 && es.channel == Mono)
			 {
			 ...
			 else if(wfh->Fmt_Channels == 2 && es.channel == Stereo)
			 {
			 ...
			 else if(wfh->Fmt_Channels == 1 && es.channel == Stereo)
			 {
			  ...
			  else if(wfh->Fmt_Channels == 1 && es.channel == Mono)
			 {
...
//*****************************DECODEING*************************************************************
//****************************************************************************************************
BOOL oggHelper::Decode(char* file_in, char* file_out)
{
	return decode_x(file_in, file_out, NULL);
}

BOOL oggHelper::Decode(char* file_in, char* file_out, WNDPROC callbackproc)
{
	return decode_x(file_in, file_out, callbackproc);
}

void* oggHelper::Decode(char* file_in, char* file_out, WNDPROC callbackproc, BOOL async)
{
	if(async)
	{
		for(int i = 0; i < MAX_THREAD_COUNT; i++)//Trying to find a free thread to use
		{
			if((WaitForSingleObject(hThread[i], 0) == WAIT_FAILED && GetLastError() == ERROR_INVALID_HANDLE)/*Not used before*/ || WaitForSingleObject(hThread[i], 0) == WAIT_OBJECT_0/*Finished and ready for use again*/)
			{
				...
				holder->ogg = file_in;
				holder->pcm = file_out;
				return hThread[i] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)decode_s, (void*)holder, NULL, NULL);
			}
		}
	}
	else
	{
		return (void*)decode_x(file_in, file_out, callbackproc);
	}
}

//The static member which we can use for the thread
int oggHelper::decode_s(void* param)
{
	OGG_StaticParam_t* sp = (OGG_StaticParam_t*)param;
	char* file_in = sp->ogg;
	char* file_out = sp->pcm;
	WNDPROC callback_proc = sp->callback_proc;
	oggHelper* oh = (oggHelper*)sp->oggObj;
    return oh->decode_x(file_in, file_out, callback_proc);
}

//The actual decode function
BOOL oggHelper::decode_x(char* file_in, char* file_out, WNDPROC callbackproc)
{
