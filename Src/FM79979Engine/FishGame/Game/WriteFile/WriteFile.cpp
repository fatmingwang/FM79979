...
void	cWriteFileManager::WriteFileThread(float e_fElpaseTime)
{
	{
		cPP11MutexHolder l_cPP11MutexHolder(m_Mutex);
...
cWriteFileManager*g_pWriteFileManager = nullptr;

cWriteBinaryFile::cWriteBinaryFile(bool e_bAddToAutoSave)
{
	...
	if (e_bAddToAutoSave)
	{
		if (!g_pWriteFileManager)
		{
			g_pWriteFileManager = new cWriteFileManager();
		}
		cPP11MutexHolder l_cPP11MutexHolder(*g_pWriteFileManager->GetMutex());
		g_pWriteFileManager->AddObject(this);
	}
}

cWriteBinaryFile::~cWriteBinaryFile()
{
	if (g_pWriteFileManager)
	{
		{
			cPP11MutexHolder l_cPP11MutexHolder(*g_pWriteFileManager->GetMutex());
			g_pWriteFileManager->RemoveObjectWithoutDelete(this);
		}
		if (g_pWriteFileManager->Count() == 0)
		{
			g_pWriteFileManager->CloseThreadAndWaitUntilFinish();
			SAFE_DELETE(g_pWriteFileManager);
		}
	}
}

bool	cWriteBinaryFile::OpenFileWhatEverFileIsExitOrNotExit(const char*e_strFileName,sRegister_Header_Struct e_sRegister_Header_Struct, bool e_bActiveWriteToFile, bool e_bRoolFilePositionToEnd)
{
...
void	cWriteBinaryFile::WriteFileUpdate(float e_fElpaseTime)
{
