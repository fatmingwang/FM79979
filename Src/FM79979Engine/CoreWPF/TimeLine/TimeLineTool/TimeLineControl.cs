...
namespace TimeLineTool
{
    ...
    internal class TimeLineDragAdorner : Adorner
    {
        ...
        internal TempDataType Data { get; set; }
    ...
    public enum TimeLineViewLevel { Seconds,Minutes, Hours, Days, Weeks, Months, Years };
    ...
    public class TimeLineControl : Canvas
    {
        ...
        public TempDataType FocusOnItem
        {
            get { return (TempDataType)GetValue(FocusOnItemProperty); }
        ...
        public static readonly DependencyProperty FocusOnItemProperty =
            DependencyProperty.Register("FocusOnItem", typeof(TempDataType), typeof(TimeLineControl), new UIPropertyMetadata(null, new PropertyChangedCallback(FocusItemChanged)));
        public static void FocusItemChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                tc.ScrollToItem(e.NewValue as TempDataType);
            }

        }

        private void ScrollToItem(TempDataType target)
        {
        ...
        #region Items
        public ObservableCollection<TempDataType> Items
        {
            get { return (ObservableCollection<TempDataType>)GetValue(ItemsProperty); }
            set { SetValue(ItemsProperty, value); }
        }

        // Using a DependencyProperty as the backing store for Items.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty ItemsProperty =
            DependencyProperty.Register("Items", typeof(ObservableCollection<TempDataType>), typeof(TimeLineControl),
            new UIPropertyMetadata(null,
        ...
        private static void OnItemsChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (tc != null)
            {
                tc.InitializeItems(e.NewValue as ObservableCollection<TempDataType>);
        ...
        {
            get { return (int)GetValue(StartDateProperty); }
            set { SetValue(StartDateProperty, value); }
        }

        // Using a DependencyProperty as the backing store for StartDate.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty StartDateProperty =
            DependencyProperty.Register("StartDate", typeof(int), typeof(TimeLineControl),
        ...
        public TimeLineControl()
        {
            ...
            //avoid time line control to move
            //AllowDrop = true;
        ...
        private void InitializeItems(ObservableCollection<TempDataType> observableCollection)
        {
            ...
            foreach (TempDataType data in observableCollection)
            {
        ...
        void Items_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                var itm = e.NewItems[0] as TempDataType;
                if (itm.StartTime == 0)
                {//newly created item isn't a drop in so we need to instantiate and place its control.
                    int duration = itm.EndTime - itm.StartTime;
                    if (Items.Count == 1)//this is the first one added
                    {
                        itm.StartTime = StartDate;
                        itm.EndTime = StartDate+duration;
                        itm.Duration = itm.EndTime - itm.StartTime;
                    }
                    else
                    {
                        var last = Items.OrderBy(i => i.StartTime).LastOrDefault();
                        if (last != null)
                        {
                            itm.StartTime = last.EndTime;
                            itm.EndTime = itm.StartTime+duration;
                            itm.Duration = itm.EndTime - itm.StartTime;
                        }
                    }
        ...
        private TimeLineItemControl CreateTimeLineItemControl(TempDataType data)
        {
            ...
            Binding duationBinding = new Binding("Duration");
            duationBinding.Mode = BindingMode.TwoWay;
            duationBinding.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;

            int timelineStart = StartDate;
            ...
            adder.SetBinding(TimeLineItemControl.DurationProperty, duationBinding);
        ...
        private void ReDrawChildren()
        {
            ...
            int start = (int)GetValue(StartDateProperty);
            Double w = 0;
            Double s = 0;
            Double e = 0;
            for (int i = 0; i < Items.Count; i++)
        ...
        public void DrawBackGround(Boolean isDrawGridUpdate)
        {
            Brush b = Background;
        ...
        private void DrawMinuteLines(Canvas grid)
        {
            ...
            TimeSpan l_TimeSpan = new TimeSpan(0, 0, (int)StartDate);
            dt.Add(l_TimeSpan);
            int startMinute = dt.Minute;
            int startSecond = dt.Second;
            ...
            if (fifteenMinSize >= MinimumUnitWidth)
            {
                ...
                DateTime nextFifteenDate = DateTime.MinValue.Add(nextFifteenGap);
                Double nextFifteenDistance = nextFifteenGap.TotalHours * UnitSize;
                DrawIncrementLines(grid, nextFifteenDate, nextFifteenDistance, new TimeSpan(0, 15, 0), fifteenMinSize, MinuteLineBrush, 0,0);
            ...
            else if (halfHourSize >= MinimumUnitWidth)
            {
                ...
                DateTime nextHalfDate = DateTime.MinValue.Add(nextHalfGap);
                Double nextHalfDistance = nextHalfGap.TotalHours * UnitSize;
                DrawIncrementLines(grid, nextHalfDate, nextHalfDistance, new TimeSpan(0, 30, 0), halfHourSize, MinuteLineBrush, 0,0);
            }
        ...
        private void DrawHourLines(Canvas grid)
        {
            ...
            TimeSpan l_TimeSpan = new TimeSpan(0, 0, (int)StartDate);
            int startMinute = l_TimeSpan.Minutes;
            int remainingMinutes = 60 - startMinute;
            int startSecond = l_TimeSpan.Seconds;
            int remainingSeconds = 60 - startSecond;
            if (remainingSeconds == 60)
                remainingSeconds = 0;
            if (startSecond != 0)
            ...
            {
                int remainingToMajor = 24 - l_TimeSpan.Hours;
                if (l_TimeSpan.Hours < 12)
                    remainingToMajor = 12 - l_TimeSpan.Hours;
                //time to our next hour
                TimeSpan firstHourGap = new TimeSpan(0, remainingMinutes, remainingSeconds);
                DateTime nextHour = DateTime.MinValue.Add(firstHourGap);
            ...
            {
                int startHour = l_TimeSpan.Hours;
                ...
                DateTime nextHalfDay = DateTime.MinValue.Add(nextHalfGap);
                Double nextHalfDistance = nextHalfGap.TotalHours * hourSize;
                DrawIncrementLines(grid, nextHalfDay, nextHalfDistance, new TimeSpan(12, 0, 0), halfDaySize, HourLineBrush, -1,0);
        ...
        private void DrawDayLines(Canvas grid)
        {
            ...
            if (daySize >= MinimumUnitWidth)
            {
                ...
                DateTime nextDay = DateTime.MinValue.Add(new TimeSpan(remainingHours, remainingMinutes, 0));


                DrawIncrementLines(grid, nextDay, firstDayDistance,
                                    increment, daySize, DayLineBrush, 7,0);
            }

        }
        private void DrawIncrementLines(Canvas grid, DateTime firstLineDate, Double firstLineDistance,
                TimeSpan timeStep, Double unitSize, Brush brush, int majorEvery, int majorEveryOffset )
        {
        ...
        void TimeLineControl_DragOver(object sender, DragEventArgs e)
        {
            ...
            if (d != null)
            {
                if (Manager != null)
                {
                    if (!Manager.CanAddToTimeLine(d.DataContext as TempDataType))
                    {
            ...
            {//GongSolutions.Wpf.DragDrop
                ...
                if (d2 != null)
                {
                    if (Manager != null)
                    {
                        if (!Manager.CanAddToTimeLine(d2 as TempDataType))
                        {
                            e.Effects = DragDropEffects.None;
                            return;
                        }
        ...
        void TimeLineControl_Drop(object sender, DragEventArgs e)
        {
            ...
            TempDataType dropData = null;
            if (dropper == null)
            {
                //dropData = e.Data.GetData(typeof(ITimeLineDataItem)) as ITimeLineDataItem;
                dropData = e.Data.GetData("GongSolutions.Wpf.DragDrop") as TempDataType;
                if (dropData != null)
                {
            ...
            var curData = dropper.DataContext as TempDataType;
        ...
        {
            var Data = adder.DataContext as TempDataType;
        ...
        {
            var curData = remover.DataContext as TempDataType;
                        ...
                        case TimeLineManipulationMode.Free:
                            ...
                            if (direction < 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    int s = (int)GetValue(StartDateProperty);
                                    gap = cLeft;
                                }
        ...
        private TimeLineItemControl GetTimeLineItemControlStartingBefore(double dateTime, ref int index)
        {
            index = -1;
            for (int i = 0; i < Items.Count; i++)
            {
        ...
        private TimeLineItemControl GetTimeLineItemControlStartingAfter(double dateTime, ref int index)
        {
            for (int i = 0; i < Items.Count; i++)
            {
                var checker = GetTimeLineItemControlAt(i);
