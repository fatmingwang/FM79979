...
#define EGL_STATUS_LOG(str) \
	LOGD("GameApp:Success: %s (%s:%d)", str, __FUNCTION__, __LINE__)
...
bool NvEGLUtil::defaultEGLChooser(EGLDisplay disp, EGLConfig& bestConfig)
{
	...
	LOGD("GameApp:Config count = %d", count);
	...
	for (i = 0; i < count; i++)
	{
		...
		LOGD("GameApp:Config[%d]: R%dG%dB%dA%d D%dS%d Type=%04x Render=%04x",
		...
		if ((match < bestMatch) || (bestIndex == -1))
		{
			bestMatch = match;
			bestIndex = i;
			LOGD("GameApp:Config[%d] is the new best config", i, configs[i]);
		}
	}
...
NvEGLUtil::~NvEGLUtil()
{
	LOGD("GameApp:**** Releasing EGL Manager object and all EGL Resources");
...
bool NvEGLUtil::setWindow(ANativeWindow* window)
{
	if (window != m_window)
	{
		LOGD("GameApp:**** Window has changed!");
		destroySurface();
	}
...
bool NvEGLUtil::createSurface()
{
	...
	if (m_surface != EGL_NO_SURFACE)
	{
		...
		LOGD("GameApp:Win=%dx%d, Surf=%dx%d", w, h, sw, sh);
		mNewWindow = true;
	}
	else
	{
...
long NvEGLUtil::getSystemTime()
{
		...
		(m_status >= NV_IS_BOUND))
	{
		...
		{
			LOGD("GameApp:Switching to NV Extension timer mode");
	...
	else
	{
		...
		{
			LOGD("GameApp:Switching to System timer mode");
			m_accumulatedTime++; // don't return zero delta in this case
		}
