...
HAVI CreateAvi(const wchar_t*fn, int frameperiod, const WAVEFORMATEX *wfx)
{
	IAVIFile *pfile;
	AVIFileInit();
	HRESULT hr = AVIFileOpen(&pfile, fn, OF_WRITE|OF_CREATE, nullptr);
	if (hr!=AVIERR_OK) {AVIFileExit(); return nullptr;}
	TAviUtil *au = new TAviUtil;
	au->pfile = pfile;
	if (wfx==nullptr) ZeroMemory(&au->wfx,sizeof(WAVEFORMATEX)); else CopyMemory(&au->wfx,wfx,sizeof(WAVEFORMATEX));
...
{
	if (avi==nullptr) return AVIERR_BADHANDLE;
...
{
	if (avi==nullptr) return AVIERR_BADHANDLE;
	if (hbm==nullptr) return AVIERR_BADPARAM;
	...
	if (au->psCompressed==0) // set the compression, prompting dialog if necessary
	{
		...
		if (opts!=nullptr) aopts[0]=opts; else aopts[0]=&myopts;
		if (ShowDialog)
		{ 
		...
		HRESULT hr = AVIMakeCompressedStream(&au->psCompressed, au->ps, aopts[0], nullptr);
		AVISaveOptionsFree(1,aopts);
...
HRESULT AddAviFrame(HAVI avi, HBITMAP hbm)
{
	...
	{ 
		...
		HRESULT hr = AVIMakeCompressedStream(&au->psCompressed, au->ps, &opts, nullptr);
		if (hr != AVIERR_OK) 
		{
			au->iserr=true; 
			return hr;
	...
	HRESULT hr = AVIStreamWrite(au->psCompressed, au->nframe, 1, dibs.dsBm.bmBits, dibs.dsBmih.biSizeImage, AVIIF_KEYFRAME, nullptr, nullptr);
	if (hr!=AVIERR_OK)
	{
		au->iserr=true; 
		return hr;
...
HRESULT AddAviAudio(HAVI avi, void *dat, unsigned long numbytes)
{
	...
	{ 
		...
		ahdr.dwQuality=(unsigned long)-1;
	...
	HRESULT hr = AVIStreamWrite(au->as,au->nsamp,numsamps,dat,numbytes,0,nullptr,nullptr);
	if (hr!=AVIERR_OK) 
	{
		au->iserr=true;
		return hr;
...
HRESULT AddAviWav(HAVI avi, wchar_t*src, unsigned long flags)
{ 
	if (avi==nullptr) return AVIERR_BADHANDLE;
	...
	{ 
		HANDLE hf=CreateFile(src,GENERIC_READ,FILE_SHARE_READ,nullptr,OPEN_EXISTING,0,nullptr);
		...
		buf = new char[size];
		unsigned long red; ReadFile(hf,buf,size,&red,nullptr);
		CloseHandle(hf);
		wav = (WavChunk*)buf;
	}
	//
	...
	unsigned long numsamps = numbytes*8 / au->wfx.wBitsPerSample;
	HRESULT hr = AVIStreamWrite(au->as,au->nsamp,numsamps,wav->dat.data,numbytes,0,nullptr,nullptr);
	if (buf!=0) delete[] buf;
	if (hr!=AVIERR_OK) {au->iserr=true; return hr;}
	au->nsamp+=numsamps; return S_OK;
}
