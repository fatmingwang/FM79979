    ...
    const char *mode;
{
    ...
    if (state->mode == GZ_READ) {
        state->start = LSEEK((FILE*)state->fd, 0, SEEK_CUR);
    ...
    gzFile file;
{
    ...
    if (LSEEK((FILE*)state->fd, state->start, SEEK_SET) == -1)
        return -1;
    gz_reset(state);
    return 0;
}
    ...
    int whence;
{
    ...
    if (state->mode == GZ_READ && state->how == COPY &&
        ...
        ret = LSEEK((FILE*)state->fd, offset, SEEK_CUR);
        if (ret == -1)
            return -1;
        state->have = 0;
        state->eof = 0;
    ...
    gzFile file;
{
    ...
    offset = LSEEK((FILE*)state->fd, 0, SEEK_CUR);
    if (offset == -1)
        return -1;
