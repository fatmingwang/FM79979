...
void glTF::Asset::ProcessMeshes( json& meshes, json& accessors )
{
    ...
    for (json::iterator meshIt = meshes.begin(); meshIt != meshes.end(); ++meshIt, ++curMesh)
    {
        ...
        for (json::iterator primIt = primitives.begin(); primIt != primitives.end(); ++primIt, ++curSubMesh)
        {
            ...
            //fuck
            FMLOG("here should be GL_TRIANGLE_STRIP")
...
inline uint32_t floatToHalf( float f )
{
    ...
    auto l_Max = max(f, 0.0f);
    x.f = min(l_Max,1.0f) * kF32toF16;
    return x.u >> 13;
}
...
int GLtoD3DTextureAddressMode(int32_t glWrapMode)
{
    //fuck
    return -1;
    //switch (glWrapMode)
...
bool glTF::Asset::Parse(const std::wstring& filepath)
{
    ...
    std::wstring fileExt = UT::GetFileExtensionName(filepath.c_str());
    ...
    if (fileExt == L"glb")
    {
        ...
        {
            FMLOG("Error:  Invalid glTF binary format\n");
            return false;
        }
        if (header.version != 2)
        {
            FMLOG("Error:  Only glTF 2.0 is supported\n");
            return false;
        }
        ...
        {
            FMLOG("Error: Expected chunk0 to contain JSON\n");
            return false;
        }
        ...
        {
            FMLOG("Error: Expected chunk1 to contain BIN\n");
            return false;
       }
    ...
    if (!root.is_object())
    {
        FMLOG("Invalid glTF file: %s\n", filepath.c_str());
        return false;
    }

    // Strip off file name to get root path to other related files
    
    m_basePath = UT::GetDirectoryWithoutFileName(filepath.c_str());
