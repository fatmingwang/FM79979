...
//	}
	...
	void	cFrameBuffer::StartDraw(bool e_bClearScreen)
	{
		...
		cGameApp::m_spOpenGLRender->m_vViewPortSize.z = (float)this->m_uiWidth;
		cGameApp::m_spOpenGLRender->m_vViewPortSize.w = (float)this->m_uiHeight;
		glBindFramebuffer(GL_FRAMEBUFFER, m_uiFramebufferID);
		if (e_bClearScreen)
		{
	...
	void	cFrameBuffer::EndDraw()
	{
		...
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		cGameApp::m_spOpenGLRender->m_vViewPortSize.x = (float)m_iOriginalViewPortSize[0];
