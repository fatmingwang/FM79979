...
namespace FATMING_CORE
{
	...
	:cBaseShader(e_strVS,e_strPS,e_strShaderName,true)
	{
		...
		m_pOriginalFrameBuffer = new cFrameBuffer((int)cGameApp::m_spOpenGLRender->m_vGameResolution.x,(int)cGameApp::m_spOpenGLRender->m_vGameResolution.y);
	...
	void	cToneMappingShader::EndDraw()
	{
		if(m_pOriginalFrameBuffer)
		{
			...
			RenderQuadWithTextureAndColorAndCoordinate(0.f,0.f,0.f,cGameApp::m_spOpenGLRender->m_vGameResolution.x,cGameApp::m_spOpenGLRender->m_vGameResolution.y,Vector4::One,l_fTextureCoordinate,Vector3::Zero,this->GetName());
	...
	void	cToneMappingShader::DebugRender()
	{
		...
		if( cGameApp::m_sucKeyData[(unsigned char)'D'] )
		{
			...
			l_Pos.y += (int)cGameApp::m_spOpenGLRender->m_vGameResolution.y/2;
			this->m_pDownSampleBrightShader->GetFrameBuffer()->DrawBuffer(l_Pos,l_Size);
			cGameApp::RenderFont(l_Pos.x,l_Pos.y,L"DownSample");
			l_Pos.x  = (int)cGameApp::m_spOpenGLRender->m_vGameResolution.x/2;
			l_Pos.y  = 0;
