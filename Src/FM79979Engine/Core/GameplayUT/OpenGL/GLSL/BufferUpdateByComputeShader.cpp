...
namespace FATMING_CORE
{
	...
	{
		SetSizeKeepData(e_uiSize);
		auto*l_pData = m_pIn->Map();
	...
	{
		auto* l_pData = m_pIn->Map();
		memcpy(l_pData, e_pData, m_pIn->GetSize());
		m_pIn->Unmap();
	}
	...
	bool cBufferUpdateByComputeShader::DispatchCompute(int e_iSizeX, int e_iSizeY, int e_iSizeZ)
	{
	...
	bool cBufferUpdateByComputeShader::CopyDatTo(char* e_pOutData,unsigned int e_uiSize)
	{
		if (!m_pOut)
		{
			if (m_pOut->GetSize() == e_uiSize)
			{
				auto l_pData = m_pOut->Map();
				memcpy(e_pOutData,l_pData, m_pIn->GetSize());
				m_pOut->Unmap();
				return true;
			}
		}
