...
class Delaunay
{
	public:
		...
		using VertexType = DelaunayVector2<T>;

		const std::vector<TriangleType>& triangulate(std::vector<VertexType>*e_pVertices)
		{
			// Store the vertices locally
			_vertices = *e_pVertices;

			// Determinate the super triangle
			T minX = _vertices[0].x;
			T minY = _vertices[0].y;
			T maxX = minX;
			T maxY = minY;
			...
			for(auto p = begin(_vertices); p != end(_vertices); p++)
			{
