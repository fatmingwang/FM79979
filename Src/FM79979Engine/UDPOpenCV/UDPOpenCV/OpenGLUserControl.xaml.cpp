...
FATMING_CORE::cOrthogonalCamera* g_pOrthogonalCamera = nullptr;
...
namespace UDPOpenCV
{
		...
		auto workItemHandler = ref new Windows::System::Threading::WorkItemHandler([this](Windows::Foundation::IAsyncAction^ action)
			{
				...
				while (action->Status == Windows::Foundation::AsyncStatus::Started)
				{
					...
					if (!g_pApp)
					{
						if (m_pOpenGLES)
							m_pOpenGLES->MakeCurrent(m_RenderSurface);
						g_pApp = new cUDPOpenCVApp(nullptr, Vector2(1920, 1080), Vector2(1920, 1080));
						g_pApp->Init();
						//g_pOrthogonalCamera = new FATMING_CORE::cOrthogonalCamera();
					...
					if (g_pApp)
					{
						...
						if (g_pOrthogonalCamera)
						{
							...
							g_pOrthogonalCamera->DrawGrid(0, 0, Vector4(0, 1, 0, 0.3f));
						}
	...
	void OpenGLUserControl::swapChainPanel_PointerMoved(Platform::Object^ sender, Windows::UI::Xaml::Input::PointerRoutedEventArgs^ e)
	{
		...
		//if (ptr->PointerDeviceType == Windows::Devices::Input::PointerDeviceType::Mouse)
		{
			...
			if (ptrPt->Properties->IsLeftButtonPressed)
			{
				...
				//l_str = "\nLeft button: " + ptrPt->PointerId;
				g_pApp->MouseDown((int)ptrPt->Position.X, (int)ptrPt->Position.Y);
				l_bLeft = true;
			}
			else
			...
			{
				g_pApp->MouseMove((int)ptrPt->Position.X, (int)ptrPt->Position.Y);
			}
			...
			if (g_pApp && g_pOrthogonalCamera)
			{
				cOrthogonalCamera* l_pOrthogonalCamera = g_pOrthogonalCamera;
				...
				l_pOrthogonalCamera->CameraUpdateByMouse(l_bLeft, l_bRight, l_iWheel, (int)ptrPt->Position.X, (int)ptrPt->Position.Y,
					Vector2(g_pApp->m_spOpenGLRender->m_vViewPortSize.Width(), g_pApp->m_spOpenGLRender->m_vViewPortSize.Height()));
			}
		}
	...
	void OpenGLUserControl::swapChainPanel_PointerReleased(Platform::Object^ sender, Windows::UI::Xaml::Input::PointerRoutedEventArgs^ e)
	{
		...
		if (g_pApp)
		{
			g_pApp->MouseUp((int)ptrPt->Position.X, (int)ptrPt->Position.Y);
		}
	}
	...
	//	{
	//		g_pApp->KeyUp((char)keycode);
	//	}
	//	else
	//	if(args->KeyStatus.WasKeyDown)
	//	{
	//		g_pApp->KeyDown((char)keycode);
	//	}
	//}
